...
const UI_TEXT = {
  zh: {
    ...
    selection: {
      ...
      title: "选品建议",
      noteText:
        "这些建议基于每周热词表现与可转化的内容信号生成。部分话题会因为连续多周保持较高热度而重复出现，点击卡片可查看对应的选品方向。",
      periodLabel: "SELECT TIME PERIOD",
      periodSelectAria: "切换选品建议日期",
      periodSummary: "当前周期：{label} · 系统推荐 {count} 个有详细证据的话题",
      forecastLabel: "趋势判断",
  ...
  en: {
    ...
    selection: {
      ...
      noteText:
        "These recommendations are based on weekly heat signals and product-ready content signals. Some topics may appear again because they remain highly relevant across multiple weeks; open a card to review the product directions.",
      periodLabel: "SELECT TIME PERIOD",
      periodSelectAria: "Switch merchandising suggestion period",
      periodSummary: "Current period: {label} · {count} evidence-backed topic(s) recommended",
...
const DYNAMIC_TEXT_PARTIALS = [
  ["已收录", "has collected"],
  ["条帖子样本", "post samples"],
  ["主要内容主题集中在", "with the strongest content theme around"],
  ["文本证据", "text evidence"],
  ["帖子/评论证据", "post/comment evidence"],
  ["AI 选品分析", "AI merchandising analysis"],
  ["兴趣DIY", "DIY hobbies"],
  ...
  ["家居生活", "Home & Living"],
  ["主题入门套装", "starter kit"],
  ["高意向搜索用户", "high-intent search users"],
  ["内容互动用户", "content-engaged users"],
  ["新品测试", "new product testing"],
  ["内容种草", "content seeding"],
  ["本地文本样本显示", "local text samples show"],
  ["已有明确讨论", "already has clear discussion"],
  ["可从高频评论问题和使用场景中提炼可售方向", "can be turned into sellable directions from frequent comment questions and usage scenarios"],
  ["基于真实评论痛点", "based on real comment pain points"],
  ["适合小批量测试", "suitable for small-batch testing"],
  ["可与内容营销联动", "works with content marketing"],
  ["待根据竞品定价", "price after competitor benchmarking"],
  ["需要进一步验证供给、定价和转化效率", "needs further validation on supply, pricing, and conversion"],
  ["先复核高互动帖子和评论问题，再用小批量商品组合测试转化", "Review high-engagement posts and comment questions first, then test conversion with small-batch product bundles"],
  ["该话题同时具备榜单热度和可阅读的帖子/评论证据", "This topic combines leaderboard heat with readable post/comment evidence"],
  ...
  ["stanley杯子为什么这么火", "Why Stanley Cups Are Popular"],
...
function localized(value) {
  if (value && typeof value === "object" && !Array.isArray(value) && ("zh" in value || "en" in value)) {
    const selected = value[appState.lang] ?? value[DEFAULT_LANG] ?? value.en ?? "";
    return appState.lang === "en" && typeof selected === "string" ? translateDynamicString(selected) : selected;
  }
...
function findTopicById(id) {
  return appState.selectionTopics.find((topic) => topic.id === id) || null;
}
...
async function loadKeywordTopicInventory() {
  try {
    const payload = await apiFetchJson("/keyword-topics/status");
    return Array.isArray(payload.available_unique_topics) ? payload.available_unique_topics : [];
  } catch {
    return [];
  }
}
...
function updateCurrentPeriod(periodKey) {
  ...
  document.querySelectorAll(".js-period-select").forEach((select) => {
    if (select.value !== appState.selectedPeriodKey) select.value = appState.selectedPeriodKey;
  });
}
...
function renderSelectionPeriodSummary() {
  const node = document.getElementById("selection-period-note");
  if (!node) return;
  const period = appState.periods.find((item) => item.key === appState.selectedPeriodKey);
  node.textContent = t("selection.periodSummary", {
    label: period?.label || appState.selectedPeriodKey || "—",
    count: appState.selectionTopics.length,
  });
}
...
function setRoute(route) {
  ...
  if (route === "selection-detail") {
    ...
    if (appState.activeDetailTopic) {
      const nextHash = `#selection-detail/${encodeURIComponent(appState.activeDetailTopic.id)}`;
...
const MERCH_PICKS_MAX_PER_WEEK = 10;
...
function uniqueSuggestionValues(suggestions, key, limit = 3) {
  ...
  suggestions.forEach((item) => {
    (item[key] || []).forEach((value) => {
      const normalized = localized(value);
      const dedupeKey = String(normalized || "").trim();
      if (!dedupeKey || seen.has(dedupeKey) || values.length >= limit) return;
      seen.add(dedupeKey);
...
function summarizePriceRange(suggestions) {
  const priceTexts = suggestions.map((item) => localized(item.pricing_hint)).filter(Boolean);
  const numbers = priceTexts.flatMap((value) => String(value || "").match(/\d+(?:\.\d+)?/g) || []).map(Number);
  if (!numbers.length) return priceTexts[0] || "—";
...
function pickSuggestionEmoji(sourceId, category = "") {
  const label = String(localized(category) || "");
  if (/蛋糕|甜品|cake|dessert/i.test(label)) return "🍰";
  if (/手机壳|phone/i.test(label)) return "📱";
  if (/黄金|首饰|珠宝|gold|jewel/i.test(label)) return "✨";
  if (/穿搭|服装|女装|配饰|fashion|outfit/i.test(label)) return "🧥";
  if (/拼豆|图纸|材料包|DIY|hobb/i.test(label)) return "🧩";
...
function findKeywordTopicForRow(row) {
  const rowTopic = String(row.topic || "").trim();
  const rowKey = normalizeTopicForMatch(rowTopic);
  if (!rowKey) return null;
  const candidates = appState.keywordTopics || [];
  let best = null;
  candidates.forEach((topic) => {
    const topicKey = normalizeTopicForMatch(topic);
    if (!topicKey) return;
    let score = 0;
    if (rowKey === topicKey) score = 100;
    else if (rowKey.includes(topicKey)) score = 80 + Math.min(topicKey.length, 20);
    else if (topicKey.includes(rowKey) && rowKey.length >= 2) score = 60 + Math.min(rowKey.length, 20);
    if (!best || score > best.score) best = { topic, score };
  });
  return best && best.score >= 60 ? best.topic : null;
}

function findStaticSourceForKeywordTopic(keywordTopic) {
  const topicKey = normalizeTopicForMatch(keywordTopic);
  return SELECTION_TOPIC_SOURCES.find((source) => {
    const terms = source.heatTerms?.length ? source.heatTerms : localizedValues(source.title);
    return terms.some((term) => {
      const termKey = normalizeTopicForMatch(term);
      return termKey && (topicKey === termKey || topicKey.includes(termKey) || termKey.includes(topicKey));
    });
  });
}

function sourceForKeywordTopic(keywordTopic, row) {
  const staticSource = findStaticSourceForKeywordTopic(keywordTopic);
  if (staticSource) return staticSource;
  const category = categorizeTopic(keywordTopic);
  return {
    id: `keyword-${normalizeTopicForMatch(keywordTopic) || hashStr(keywordTopic)}`,
    title: { zh: keywordTopic, en: translateDynamicString(keywordTopic) },
    subtitle: { zh: "文本证据话题", en: "Evidence-backed topic" },
    summary: {
      zh: `「${keywordTopic}」进入当前周 Top100，且本地已收录具体帖子/评论文本。系统建议基于这些真实内容样本进一步判断用户需求、消费场景和可转化商品方向。`,
      en: `"${translateDynamicString(keywordTopic)}" appears in this week's Top 100 and has collected post/comment text in the local dataset. The system recommends reviewing it for demand signals, usage scenarios, and product opportunities.`,
    },
    forecast: { zh: row.surge?.includes("搜索飙升") ? "+搜索飙升" : "+上榜可观察", en: row.surge?.includes("搜索飙升") ? "+Search surge" : "+Leaderboard signal" },
    keywords: [
      { emoji: "🔥", label: { zh: keywordTopic, en: translateDynamicString(keywordTopic) } },
      { emoji: "📄", label: { zh: "帖子/评论已收录", en: "Posts/comments collected" } },
      { emoji: "🏷️", label: { zh: getCategoryLabel(category), en: getCategoryLabel(category) } },
    ],
    heatTerms: [keywordTopic, row.topic],
    quote: {
      zh: "该话题不是只看热度，而是同时具备榜单信号和可阅读的文本证据。",
      en: "This topic is selected because it combines leaderboard heat with readable text evidence.",
    },
    suggestions: buildGenericSuggestions(keywordTopic, category),
  };
}

function buildGenericSuggestions(topic, category) {
  const topicEn = translateDynamicString(topic);
  const categoryLabel = getCategoryLabel(category);
  return [
    {
      product_name: `${topic}主题入门套装`,
      category: categoryLabel,
      target_users: ["高意向搜索用户", "内容互动用户"],
      use_scenarios: ["新品测试", "内容种草", "组合销售"],
      why_recommended: `「${topic}」已有具体帖子/评论文本，可先从高频需求词、评论问题和使用场景里提炼最小可售商品。`,
      evidence: [`${topic} 已收录具体帖子/评论文本`, "进入当前周热词 Top100"],
      selling_points: ["围绕真实评论痛点设计", "适合小批量测试", "可与内容营销联动"],
      pricing_hint: "待根据竞品定价",
      risk_note: "需要先复核评论样本，避免只根据热度做选品。",
    },
    {
      product_name: `${topic}场景化礼品/组合包`,
      category: categoryLabel,
      target_users: ["送礼用户", "场景消费用户"],
      use_scenarios: ["节日送礼", "兴趣消费", "社交分享"],
      why_recommended: `${topicEn} 的文本样本可用于识别具体场景，把单品变成更容易理解的组合方案。`,
      evidence: ["本地文本数据可支持进一步分析", "榜单热度提供需求入口"],
      selling_points: ["场景表达清楚", "提高客单价", "降低用户选择成本"],
      pricing_hint: "按组合复杂度分层",
      risk_note: "组合包需控制库存和履约复杂度。",
    },
  ];
}

function sourceHasDetailedEvidence(source) {
  return (source.suggestions || []).some((item) => {
    const evidenceCount = Array.isArray(item.evidence) ? item.evidence.filter(Boolean).length : 0;
    return evidenceCount > 0 && (item.why_recommended || item.selling_points?.length || item.target_users?.length);
  });
}
...
function rowMatchesSource(row, source) {
  ...
  return terms.some((term) => {
    ...
    return normalized && (topic === normalized || topic.includes(normalized) || normalized.includes(topic));
  });
}

function findSourceHeatMetrics(source, rows = appState.currentRows) {
  const matchedRows = rows.filter((row) => rowMatchesSource(row, source));
  const bestRow = matchedRows.slice().sort((a, b) => (a.rank || 999) - (b.rank || 999))[0] || null;
  const topIndex = matchedRows.reduce((best, row) => Math.max(best, Number(row.index) || 0), 0);
  return {
    topIndex,
    label: compactHeatValue(topIndex),
    bestRank: bestRow?.rank || 999,
    matchedTerm: bestRow?.topic || "",
  };
}
...
function selectionTopicFromAnalysis(source, meta = {}) {
  const suggestions = source.suggestions || [];
  const leadScenes = uniqueSuggestionValues(suggestions, "use_scenarios", 2).join(" / ") || "—";
  const heatValue = Number(meta.heat ?? source.heat ?? 0);
  const contentDirection = localized(source.keywords?.[1]?.label) || localized(source.keywords?.[0]?.label) || localized(source.title);
  return {
    id: meta.id || source.id,
    category: meta.category || suggestions[0]?.category || localized(source.subtitle),
    heat: compactHeatValue(heatValue),
    heatScore: heatValue,
    matchedRank: meta.rank || meta.matchedRank || 999,
    matchedTerm: meta.matched_term || meta.matchedTerm || "",
    keywordTopic: meta.topic || source.topic,
    title: source.title,
    ...
    quote: source.quote,
    reason: buildReasonFromSource({ ...source, suggestions }),
    stats: [
      {
        k: { zh: "建议商品", en: "Suggested SKUs" },
        v: { zh: `${suggestions.length} 个`, en: `${suggestions.length}` },
      },
      { k: { zh: "内容方向", en: "Content angle" }, v: contentDirection },
      { k: { zh: "热度判断", en: "Heat signal" }, v: source.forecast },
    ],
    ...
    strategy: source.strategy || buildStrategyFromSource({ ...source, suggestions }),
  };
}

function selectionTopicFromMerchListItem(item) {
  return {
    id: item.id,
    category: item.category,
    heat: compactHeatValue(Number(item.heat) || 0),
    heatScore: Number(item.heat) || 0,
    matchedRank: item.rank || 999,
    matchedTerm: item.matched_term || "",
    keywordTopic: item.topic,
    title: item.title,
    ...
    tags: item.keywords || [],
    quote: {
      zh: "围绕用户已经表现出兴趣的场景，优先选择容易理解、容易展示、容易组合销售的商品方向。",
      en: "Focus on product directions that are easy to understand, easy to demonstrate, and easy to bundle around scenarios users already care about.",
    },
    reason: item.summary,
    stats: [],
    products: [],
...
async function loadWeeklyMerchTopics() {
  try {
    const payload = await apiFetchJson(`/merch/topics?week=${encodeURIComponent(appState.selectedPeriodKey)}&limit=6`);
    const items = Array.isArray(payload.items) ? payload.items : [];
    return items.map(selectionTopicFromMerchListItem);
  } catch {
    return buildSelectionTopics();
  }
}

async function loadMerchTopicDetail(topic) {
  if (!topic?.needsDetail) return topic;
  const cacheKey = topic.keywordTopic || topic.id;
  if (appState.selectionDetailCache.has(cacheKey)) return appState.selectionDetailCache.get(cacheKey);
  const payload = await apiFetchJson(`/merch/topics/${encodeURIComponent(topic.keywordTopic)}`);
  const detailTopic = selectionTopicFromAnalysis(payload, {
    id: topic.id,
    topic: topic.keywordTopic,
    heat: topic.heatScore,
    rank: topic.matchedRank,
    matched_term: topic.matchedTerm,
    category: topic.category,
  });
  appState.selectionDetailCache.set(cacheKey, detailTopic);
  appState.selectionTopics = appState.selectionTopics.map((item) => (item.id === topic.id ? detailTopic : item));
  return detailTopic;
}

function buildSelectionTopics() {
  const seenTopics = new Set();
  const topics = appState.currentRows.flatMap((row) => {
    const keywordTopic = findKeywordTopicForRow(row);
    if (!keywordTopic || seenTopics.has(keywordTopic)) return [];
    seenTopics.add(keywordTopic);
    const source = sourceForKeywordTopic(keywordTopic, row);
    if (!sourceHasDetailedEvidence(source)) return [];
    const heatMetrics = findSourceHeatMetrics(source, [row]);
    ...
    return [{
      ...
      heat: compactHeatValue(row.index),
      heatScore: Number(row.index) || heatMetrics.topIndex,
      matchedRank: row.rank || heatMetrics.bestRank,
      matchedTerm: row.topic,
      keywordTopic,
      title: source.title,
      subtitle: source.subtitle,
      desc: source.summary,
      ...
      stats: [
        ...
        { k: { zh: "内容方向", en: "Content angle" }, v: localized(source.keywords?.[1]?.label) || localized(source.keywords?.[0]?.label) || localized(source.title) },
        { k: { zh: "热度判断", en: "Heat signal" }, v: source.forecast },
      ],
  ...
  topics.sort((a, b) => (a.matchedRank || 999) - (b.matchedRank || 999) || (b.heatScore || 0) - (a.heatScore || 0));
  return topics.slice(0, MERCH_PICKS_MAX_PER_WEEK);
}
...
function selectionTopicCategory(topic) {
  if (topic.category && CATEGORY_ORDER.includes(topic.category)) return topic.category;
...
async function openDetailModal(topic) {
  let detailTopic = topic;
  try {
    detailTopic = await loadMerchTopicDetail(topic);
  } catch {
    detailTopic = topic;
  }
  appState.activeDetailTopic = detailTopic;
  renderDetailPage(detailTopic);
...
async function refreshSelectionTopics() {
  const requestId = ++appState.selectionRequestId;
  const grid = document.getElementById("selection-grid");
  if (grid) grid.innerHTML = `<div class="card card-pad selection-empty">${t("hot.loading")}</div>`;
  const topics = await loadWeeklyMerchTopics();
  if (requestId !== appState.selectionRequestId) return;
  appState.selectionTopics = topics;
  renderSelectionPeriodSummary();
  if (appState.route === "selection-detail" && appState.activeDetailTopic) {
    ...
    if (!next) {
      ...
      let detailTopic = appState.activeDetailTopic;
      if (!detailTopic.products?.length) {
        try {
          detailTopic = await loadMerchTopicDetail(next);
        } catch {
          detailTopic = next;
        }
      } else {
        appState.selectionDetailCache.set(detailTopic.keywordTopic || detailTopic.id, detailTopic);
        appState.selectionTopics = appState.selectionTopics.map((topic) => (topic.id === detailTopic.id ? detailTopic : topic));
      }
      appState.activeDetailTopic = detailTopic;
      renderDetailPage(detailTopic);
      scheduleDetailProductsPanelHeightSync();
...
async function boot() {
  ...
  await detectApiBase();
  appState.keywordTopics = await loadKeywordTopicInventory();
  ...
  window.addEventListener("hashchange", () => {
    ...
    if (h.startsWith("selection-detail/")) {
      const topic = findTopicById(decodeURIComponent(h.split("/")[1] || ""));
  ...
  if (location.hash.startsWith("#selection-detail/")) {
    const topic = findTopicById(decodeURIComponent(location.hash.replace("#selection-detail/", "")));
    if (topic) openDetailModal(topic);
    else setRoute("selection");
