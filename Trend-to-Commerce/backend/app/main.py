...
from backend.app.services.merch_analysis_service import list_weekly_merch_topics, read_cached_analysis
...
@app.get("/api/merch/topics")
def merch_topics(week: str = Query(..., min_length=3, max_length=64), limit: int = Query(6, ge=1, le=10)):
    try:
        return {"week": week, "items": list_weekly_merch_topics(week, limit=limit)}
    except FileNotFoundError as exc:
        handle_not_found(exc)


@app.get("/api/merch/topics/{topic}")
def merch_topic_detail(topic: str):
    try:
        return read_cached_analysis(topic)
    except FileNotFoundError as exc:
        handle_not_found(exc)
